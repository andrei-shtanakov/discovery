...
- [x] Вывод гейта в брифе удовлетворял проверку, которая его породила @owner:github:andrei-shtanakov @id:gate-findings-self-satisfying
      **Найдено и исправлено 2026-08-20 (PR #TBD).** GC-05(engineer) ищет id
...
- [x] Engineer-фрейм не может достичь `readiness: ready`: `feasibility_review` рантайм не выводит @owner:github:andrei-shtanakov @id:feasibility-review-not-derived
      **Сделано 2026-08-20 (PR #TBD).** Ключ с префиксом `None` выводится из
...
- [ ] Проверить своим прогоном, что resume maestro применяет правку `project.yaml`, а не старый scope из `state.db` @owner:github:andrei-shtanakov @id:watch-maestro-198
      **`maestro#198` закрыт 2026-08-19 без единого комментария**, поэтому что
      именно починено — неизвестно. Ожидание доставлено, но резолюции нет: тег
      `@blocked_by:` снят, потому что снаружи нас больше ничто не блокирует —
      осталась наша собственная проверка. Стоило двух прогонов: правка
      конфигурации выглядела применённой и не была, так что верить закрытию
      issue на слово здесь дороже, чем проверить. Проверять перед следующим
      прогоном, а не после.

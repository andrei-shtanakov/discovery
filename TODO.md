...
- [ ] Фаза 3 (grounding): чтение `../prograph-vault` перед интервью, чтобы не спрашивать уже известное; `traces_to` на KB @owner:github:andrei-shtanakov @trigger:"S1 показал медианный requirement recall ≥ 0.60 на пяти свежих живых прогонах И появился пилот с KB в prograph-vault, где повторные вопросы подтверждённо стали проблемой по времени или стоимости интервью" @id:phase-3-grounding @epic:eco.discovery-runtime
      **Решение по baseline 2026-09-14: не открывать.** Блокер `l3-quality-benchmark`
      доставлен (PR #38), baseline обновлён в `discovery-test` (commit `b7313b9`):
      медианный `extraction_recall` S1 — 0.398 [0.340–0.485] на пяти свежих
      валидных прогонах, `unanchored_entry_rate` 0.483. Узкое место — полнота захвата, а не лишние
...
- [x] Оркестрированный запуск: приём upstream-брифа (`start --upstream`) и caller-assigned `--session-id` @owner:github:andrei-shtanakov @id:orchestrated-start-upstream-and-session-id @epic:eco.discovery-runtime
      **Сделано (PR #50).** Приёмка п.1 — сквозной тест по реальному банку:
      ...
      недостающих куска лежат в `cmd_start`.
      **`--upstream <file>` (блокирует E2).** Сейчас `traces_to` разрешается от
      каталога сессии (`gate.render_and_gate(..., _session_dir)`), а
      `gate_check._resolve_ref` отбрасывает абсолютные и выходящие за корень
      пути, поэтому файл из репо-цели из сессии не разрешим — публичного способа
      принять upstream у engineer-фрейма нет. Discovery копирует источник в
      сессию под **фиксированным** именем `upstream.md` и ставит его первым
      элементом `traces_to`; итоговый бриф несёт то же переносимое имя, а
      вызывающий держит свою durable-копию рядом с `brief --out`.
      Имя фиксированное, а не `<basename>` источника (как предполагал дизайн
      соседа): при совпадении basename с именем итогового брифа `traces_to`
      разрешился бы **в сам бриф** — GC-16 зелёный, GC-05(engineer) сверяет
      документ сам с собой, все Must-FR «упомянуты». Молчаливый ложный pass
      дороже потери информативности имени; провенанс источника всё равно живёт
      в леджере вызывающего (`interview.upstream_blob`).
      Отсюда же четыре инварианта приёма: источник проверяется **до** создания
      сессии (`schema: discovery-brief`, `interview.frame: customer`,
      `status: approved`, ноль error-findings текущего `gate_check`) — иначе
      можно пройти почти всё интервью на неправильном источнике; `header.json`
      пишется **последним**, как commit-marker сессии, иначе crash оставит
      читаемую сессию со ссылкой на отсутствующий файл; имя `upstream.md`
      зарезервировано и повторное `--traces-to upstream.md` отклоняется как
      неоднозначное; `brief --out .../upstream.md` для такой сессии
      отклоняется — фиксированное имя убирает обычную коллизию, но произвольный
      `--out` всё ещё позволяет самоссылку.
      **`--session-id <id>` (не блокирует, но убирает класс отказов).**
      Вызывающий пишет id write-ahead до вызова, и сиротство «сессия создана,
      запись не успела» исчезает по построению — у соседа сегодня вместо этого
      аварийный вход с присоединением по форме отрендеренного брифа. Валидация —
      существующий `_validate_session_id`, занятый id отказывает **без записи**
      (`mkdir(exist_ok=False)`, атомарно, без гонки «проверил-создал»): сегодня
      повторный id молча перезаписал бы `header.json` живой сессии.
...
Намеренно не чекбоксы: решения принимаются за пределами discovery.
...
- [x] Метаданные сессии (`frame`, `target`, `traces_to`, `source_pin`) в публичном envelope @owner:github:andrei-shtanakov @id:session-metadata-in-envelope
      **Решено 2026-09-18 (решение владельца): не вводить метаданные сессии
      восьмым top-level ключом в envelope v1.** Вызывающий уже знает `frame`,
      `target`, `traces_to` и caller-assigned `session_id` из write-ahead
      запроса, а присоединение подтверждает по отрендеренному брифу. Пересмотр —
      при появлении второго независимого потребителя или доказанном случае,
      который нельзя проверить этими фактами.
      Запрошено п.3 `discovery#49`; сам запрос помечал пункт неблокирующим для
      E2. Цена ввода — правка публичного контракта: «ровно семь ключей в порядке
      §7» зафиксированы и в `protocol.Envelope.to_json`, и в README «What a
      caller reads».

- [x] Определить поведение стадии Need без реального стейкхолдера @owner:github:andrei-shtanakov @id:solo-mode-decision-need-requires-stakeholder
      **Решено (PR #44; решение владельца 2026-09-13): соло-режим не вводить.** Режим Need
      ...
      «Последствия» §4. Допустимое значение `interview.sessions[].medium: self`
      в вендоренном контракте остаётся provenance готовых внешних брифов, а не
      режимом запуска Need; runtime это поле не читает и не рендерит. Источник:
